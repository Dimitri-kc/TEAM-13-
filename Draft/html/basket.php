...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
  ...
  if ($action === "add") {
    ...
    $qty = as_positive_int($_POST["qty"] ?? 1, 1);

    if ($id > 0) {
      $_SESSION["cart"][$id] = (int)($_SESSION["cart"][$id] ?? 0) + $qty;
    }

    $back = (string)($_POST["return_to"] ?? "basket.php");
    $isRelative = str_starts_with($back, "/") || !preg_match('~^[a-zA-Z][a-zA-Z0-9+.-]*:~', $back);
    if (!$isRelative) $back = "basket.php";
  ...
  if ($action === "update") {
    ...
    if (isset($_POST["remove_id"])) {
      ...
      if ($removeId > 0) unset($_SESSION["cart"][$removeId]);
    ...
    if (is_array($qtyArr)) {
      foreach ($qtyArr as $id => $q) {
        ...
        $q = is_string($q) && preg_match('/^-?\d+$/', $q) ? (int)$q : (is_int($q) ? $q : 0);
...
if (count($cart) > 0) {
  $ids = array_values(array_filter(array_map('intval', array_keys($cart)), fn($x) => $x > 0));
  $ids = array_values(array_unique($ids));

  if (count($ids) > 0) {
    $placeholders = implode(",", array_fill(0, count($ids), "?"));

    $sql = "SELECT product_id, product_name, price, image_path FROM products WHERE product_id IN ($placeholders)";
...
<head>
  ...
  <style>
    .basket-empty-title{margin:0 0 6px; font-size:20px;}
    .basket-empty-sub{margin:0 0 16px; color:#555;}
    .basket-empty-box{
      border:1px dashed #d8d8d8; border-radius:16px; padding:26px 22px;
      color:#555; background:#fff; text-align:center;
    }
    .basket-empty-link{
      display:inline-block; margin-top:14px; color:#000; text-decoration:underline;
    }
  ...
  <section class="basket-layout">
    <div class="basket-items">
      ...
      <?php if (!$hasItems): ?>
        ...
        <div>
          <div class="basket-empty-box">
            <div class="basket-empty-title">No items in your basket yet.</div>
            <div class="basket-empty-sub">Go to categories and add a product.</div>
          </div>
          <div class="actions">
          ...
          <?php foreach ($cartProducts as $p): ?>
            <div class="basket-item">
              ...
              <div class="item-details">
                ...
                <p class="price"><?= money((float)$p["price"]) ?></p>
                ...
                <div class="quant-controls">
                  <input
                    ...
                    value="<?= (int)$p["qty"] ?>"
                  />
                  <span style="color:#555; margin-left:10px;">Line: <?= money((float)$p["line"]) ?></span>
                </div>
    ...
    <aside class="basket-summary">
      ...
      <div class="checkout-info">
        ...
        <p>On the next page you’ll be asked to log in or sign up if this is your first time, so you can confirm as a guest.</p>
        <button class="checkout-btn" <?= $hasItems ? "" : "disabled" ?> onclick="<?= $hasItems ? "window.location.href='checkout.php'" : "return false;" ?>">Checkout</button>
